...
2. **計算機**：底部導覽多了「計算機」按鈕，裡面有兩個分頁：
   ...
   - 找零計算機：輸入應收金額、顧客給的錢（或直接點 100／500／1000 等面額快速加總），
     自動算出要找多少錢。
3. **換成自訂 Logo 圖示**：`icons/` 資料夾換成你提供的「Shuangzi Sticky Rice」圖示。
4. **加快開啟速度**：App 需要的 React、Babel 這些「零件」，現在第一次打開後會被
   手機記住（快取），下次再打開就不用重新下載，速度會明顯變快。**第一次打開還是
   會需要等個幾秒鐘**（要下載這些零件），這是正常的；之後每次打開才會感覺到變快。
   如果之前已經加到主畫面過，記得整個資料夾重新上傳蓋掉舊檔後，把 App 完全關掉
   （不是切到背景，是真的關掉）再重新打開一次，才會套用到最新版本。
