...
import {
  ...
  Search, AlertTriangle, Check, Loader2, FileText, Calculator
} from 'lucide-react';
...
const COLUMN_DEFS = [
  ...
  { key: 'cashPaidPurchase', label: '└現金支付進貨' },
  { key: 'cash', label: '實際現金' },
...
// 老闆有時會直接用「今天收到的現金」當場付給廠商，這種進貨會讓收攤後點到的現金變少，
// 所以這部分要自動加回營業額，避免跟「本日進貨」欄位重複扣兩次。
// 晚上另外補貨（不是用今天現金付的）不會被加回，維持正常扣款。
function getCashPaidPurchaseTotal(r) {
  const items = r.purchaseItems || {};
  const flags = r.purchaseCashPaid || {};
  return Object.keys(items).reduce((s, name) => s + (flags[name] ? (Number(items[name]) || 0) : 0), 0);
}
function calcTotalRevenue(r) {
  return (Number(r.cash) || 0) + (Number(r.linePay) || 0) + (Number(r.uberEats) || 0) + getCashPaidPurchaseTotal(r);
}
...
function cellValue(r, key) {
  switch (key) {
    ...
    case 'cashPaidPurchase': return fmtMoney(getCashPaidPurchaseTotal(r));
  ...
  return (
    <div style={{ background: 'var(--cream)', minHeight: '100vh' }} className="app-root">
      ...
      <div className="app-ui">
        ...
        <BottomNav view={view} setView={(v) => { sound.tap(); setView(v); }} onCalculator={() => { sound.tap(); setCalcOpen(true); }} />
      </div>
...
function BottomNav({ view, setView, onCalculator }) {
  const items = [
  ...
  return (
    <nav className="bottom-nav">
      ...
      <button className="nav-btn" onClick={onCalculator}>
...
function EntryCard({ record: r, onEdit, onDelete }) {
  ...
  const cashPaidTotal = getCashPaidPurchaseTotal(r);
  return (
    <div className="entry-card">
      ...
      <div className="entry-grid">
        ...
        {cashPaidTotal > 0 && <EntryMini label="現金支付進貨" value={cashPaidTotal} />}
        {hasAnySales(r) ? (
...
function ReportView({ cursor, setCursor, records, byDate, presets, keyword, setKeyword, onPrint, onImportBackup, onEdit, onDelete }) {
  ...
  function exportCSV() {
    sound.tap();
    const headers = ['日期', '今日營業額', 'Uber Eats', 'Line Pay', '銷售數量', '本日進貨', '現金支付進貨', '實際現金', '今日總收入', '今日淨收入', '備註'];
    ...
    monthRecords.slice().sort((a, b) => a.date.localeCompare(b.date)).forEach(r => {
      const row = [
        ...
        getPurchaseCost(r), getCashPaidPurchaseTotal(r), r.cash || 0, calcTotalRevenue(r), calcNetIncome(r),
        `"${(r.note || '').replace(/"/g, '""')}"`
      ];
      lines.push(row.join(','));
    });
...
function falseFilled(names) { return Object.fromEntries(names.map(n => [n, false])); }
...
function EntryFormModal({ state, presets, onAddPurchasePreset, onAddProductPreset, onDeletePurchasePreset, onDeleteProductPreset, onClose, onSave }) {
  const initial = state.mode === 'edit'
    ? {
        ...
        purchaseCashPaid: { ...falseFilled(presets.purchaseNames), ...(state.data.purchaseCashPaid || {}) },
      }
    : {
        ...
        purchaseCashPaid: falseFilled(presets.purchaseNames),
      };
  ...
  const cashPaidTotal = getCashPaidPurchaseTotal(form);
  ...
  function setPurchaseCashPaid(name, val) { setForm(f => ({ ...f, purchaseCashPaid: { ...f.purchaseCashPaid, [name]: val } })); }
  ...
  function handleAddPurchaseItem() {
    ...
    setForm(f => ({
      ...f,
      purchaseItems: { ...f.purchaseItems, [trimmed]: f.purchaseItems[trimmed] ?? 0 },
      purchaseCashPaid: { ...f.purchaseCashPaid, [trimmed]: f.purchaseCashPaid[trimmed] ?? false },
    }));
  }
  ...
  function handleDeletePurchaseItem(name) {
    ...
    setForm(f => {
      ...
      delete next[name];
      const nextFlags = { ...f.purchaseCashPaid };
      delete nextFlags[name];
      return { ...f, purchaseItems: next, purchaseCashPaid: nextFlags };
    });
  }
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-panel" onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-body">
          ...
          <FormField label="今日營業額（收入細項加總，含現金支付進貨自動加回）">
          ...
          <div className="calc-box">
            ...
            {cashPaidTotal > 0 && (
              <div className="calc-row"><span>└ 其中現金支付進貨自動加回</span><b>NT$ {fmtMoney(cashPaidTotal)}</b></div>
            )}
          ...
          <div className="form-grid">
            <FormField label="實際收入現金（收攤後實際點到的現金）"><MoneyInput value={form.cash} onChange={v => set('cash', v)} /></FormField>
          ...
          <div className="field-hint">
            如果這筆是老闆當場用「今天收到的現金」直接付給廠商的，請勾選「今日現金支付」，
            系統會自動把這筆加回營業額，避免現金被重複扣兩次。晚上另外補貨（不是用今天現金付的）不要勾。
          </div>
          <div className="form-grid">
            {presets.purchaseNames.map(name => (
              ...
              >
                ...
                <label className="cash-flag-row">
                  <input type="checkbox" checked={!!form.purchaseCashPaid[name]} onChange={e => setPurchaseCashPaid(name, e.target.checked)} />
                  <span>今日現金支付</span>
                </label>
              </FormField>
            ))}
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-panel calc-modal" onClick={e => e.stopPropagation()}>
        <div className="modal-header">
          <div className="modal-title"><Calculator size={17} style={{ verticalAlign: -3, marginRight: 6 }} />計算機</div>
          <button className="icon-btn" onClick={onClose}><X size={18} /></button>
        </div>
        <div className="modal-body">
          <div className="seg">
            <button className={`seg-btn ${tab === 'basic' ? 'seg-btn-active' : ''}`} onClick={() => setTab('basic')}>一般計算機</button>
            <button className={`seg-btn ${tab === 'change' ? 'seg-btn-active' : ''}`} onClick={() => setTab('change')}>找零計算機</button>
          </div>
          {tab === 'basic' ? <StandardCalculator /> : <ChangeCalculator />}
        </div>
      </div>
    </div>
  );
...
function roundCalc(n) { return Math.round(n * 1e8) / 1e8; }
...
function StandardCalculator() {
  ...
  function inputDigit(d) {
    sound.step();
    if (waiting || display === '錯誤') { setDisplay(d); setWaiting(false); return; }
    setDisplay(display === '0' ? d : display + d);
  }
  function inputDecimal() {
    sound.step();
    if (waiting || display === '錯誤') { setDisplay('0.'); setWaiting(false); return; }
  ...
  function percent() {
    sound.step();
    setDisplay(d => String(roundCalc(Number(d) / 100)));
  }
  ...
  function chooseOperator(nextOp) {
    ...
    if (stored == null) {
      ...
      const result = calc(stored, inputValue, operator);
      setStored(result);
      setDisplay(Number.isNaN(result) ? '錯誤' : String(result));
    }
    setWaiting(true);
    setOperator(nextOp);
  }
  ...
  const pad = [
    ...
    { label: '÷', onClick: () => chooseOperator('/'), kind: 'op', active: operator === '/' },
    { label: '7', onClick: () => inputDigit('7'), kind: 'num' },
    { label: '8', onClick: () => inputDigit('8'), kind: 'num' },
    { label: '9', onClick: () => inputDigit('9'), kind: 'num' },
    { label: '×', onClick: () => chooseOperator('*'), kind: 'op', active: operator === '*' },
    { label: '4', onClick: () => inputDigit('4'), kind: 'num' },
    { label: '5', onClick: () => inputDigit('5'), kind: 'num' },
    { label: '6', onClick: () => inputDigit('6'), kind: 'num' },
    { label: '−', onClick: () => chooseOperator('-'), kind: 'op', active: operator === '-' },
    { label: '1', onClick: () => inputDigit('1'), kind: 'num' },
    { label: '2', onClick: () => inputDigit('2'), kind: 'num' },
    { label: '3', onClick: () => inputDigit('3'), kind: 'num' },
    { label: '+', onClick: () => chooseOperator('+'), kind: 'op', active: operator === '+' },
    { label: '±', onClick: toggleSign, kind: 'fn' },
    { label: '0', onClick: () => inputDigit('0'), kind: 'num' },
    { label: '.', onClick: inputDecimal, kind: 'num' },
      ...
      <div className="calc-pad">
        {pad.map((b, i) => (
          <button
            key={i}
            type="button"
            className={`calc-btn calc-btn-${b.kind} ${b.active ? 'calc-btn-active' : ''}`}
            onClick={b.onClick}
          >
            {b.label}
          </button>
...
function ChangeCalculator() {
  ...
  const change = (Number(received) || 0) - (Number(due) || 0);
  const hasInput = due !== '' || received !== '';

  function addBill(v) {
    sound.step();
    setReceived(r => String((Number(r) || 0) + v));
  }
  function reset() {
    sound.tap();
    setDue('');
  ...
  return (
    <div>
      <FormField label="應收金額"><MoneyInput value={due} onChange={setDue} /></FormField>
      <FormField label="顧客給的錢"><MoneyInput value={received} onChange={setReceived} /></FormField>
      <div className="bill-chip-row">
        {[50, 100, 200, 500, 1000].map(v => (
          <button key={v} type="button" className="bill-chip" onClick={() => addBill(v)}>+{v}</button>
        ))}
        <button type="button" className="bill-chip bill-chip-clear" onClick={reset}>清空</button>
      </div>
      <div className="calc-box" style={{ marginTop: 14 }}>
        <div className="calc-row">
          ...
          {!hasInput ? (
            ...
            <b className="neg">還差 NT$ {fmtMoney(Math.abs(change))}</b>
          ) : (
            <b>NT$ {fmtMoney(change)}</b>
          )}
        </div>
      </div>
    </div>
...
function GlobalStyle() {
  return (
    <style>{`
      ...
      .nav-btn { display: flex; flex-direction: column; align-items: center; gap: 3px; background: none; border: none; color: var(--ink-soft); font-size: 11px; padding: 4px 10px; cursor: pointer; }
      ...
      .cash-flag-row { display: flex; align-items: center; gap: 5px; font-size: 11px; color: var(--ink-soft); margin-top: 6px; cursor: pointer; }
      .cash-flag-row input { accent-color: var(--accent); width: 13px; height: 13px; }
      .field-hint { font-size: 11.5px; color: var(--ink-soft); background: #FBF6EC; border: 1px dashed var(--tea); border-radius: 10px; padding: 8px 10px; line-height: 1.5; margin-top: 4px; }
      ...
      .calc-modal .modal-body { padding-bottom: 22px; }
      .calc-display { background: #EFE9DC; border: 1px solid var(--line); border-radius: 12px; padding: 18px 16px; text-align: right; font-size: 30px; font-weight: 700; color: var(--wood); word-break: break-all; margin-top: 12px; }
      .calc-pad { display: grid; grid-template-columns: repeat(4, 1fr); gap: 8px; margin-top: 10px; }
      .calc-btn { border: none; border-radius: 12px; padding: 15px 0; font-size: 17px; font-weight: 700; cursor: pointer; background: var(--paper); border: 1px solid var(--line); color: var(--ink); }
      .calc-btn-fn { background: #EFE6D8; color: var(--wood); }
      .calc-btn-op { background: #F6E4DD; color: var(--accent-dark); }
      .calc-btn-op.calc-btn-active { background: var(--accent); color: #fff; }
      .calc-btn-eq { background: var(--accent); color: #fff; }
      .calc-btn:active { transform: scale(0.96); }

      .bill-chip-row { display: flex; flex-wrap: wrap; gap: 8px; margin-top: 10px; }
      .bill-chip { border: 1px solid var(--line); background: var(--paper); color: var(--accent-dark); border-radius: 999px; padding: 7px 14px; font-size: 13px; font-weight: 700; cursor: pointer; }
      .bill-chip-clear { color: var(--ink-soft); font-weight: 400; }
